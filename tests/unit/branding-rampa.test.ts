 ...
 * a catraca deixaria de calibrar contra a régua. Lendo do arquivo, mudar a rampa azul quebra
...
function stopsAzuisDoCss(): string[] {
...
describe("rampaDeSemente — catraca de calibração contra o design system", () => {
  const esperados = stopsAzuisDoCss();
  ...
  it("lê 11 stops distintos do globals.css (guarda de vacuidade)", () => {
    ...
    expect(esperados[K]).toBe("#1447e6");
  });

  it("reproduz os 11 stops azuis a partir de #1447e6 com Δ ≤ 2/255 por canal", () => {
    const derivada = rampaDeSemente("#1447e6");
